...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuite with ShouldMatchers with BeforeAndAfterAll with ImplicitSender 
{
  ...
  test("proper inserts and lookups") {
    val topNode = system.actorOf(Props[BinaryTreeSet], "TestInserts+Lookups")
  ...
  test("Nested inserts") {
    val topNode = system.actorOf(Props[BinaryTreeSet], "NestedInserts")
    ...
    topNode ! Contains(testActor, id = 9, 3)
    expectMsg(ContainsResult(9, false))
    topNode ! Contains(testActor, id = 10, -3)
    expectMsg(ContainsResult(10, false))
  }

  test("Remove") {
    val topNode = system.actorOf(Props[BinaryTreeSet], "Remove")
