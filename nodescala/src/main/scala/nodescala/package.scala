...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      for (future <- fs) {
        future.onComplete {
          case Success(value) => promise.tryComplete(Try(value))
          case Failure(throwable) => promise.tryFailure(throwable)
        }
      }
      promise.future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      Future { f(sub.cancellationToken) }
      sub
    }

  }

  def doAll[S](fs: List[Future[S]]): Future[List[S]] = {
    async {
      var futures = fs
      val list = ListBuffer[S]()
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      if (f.isCompleted) {
        Await.result(f, Duration(1, TimeUnit.MILLISECONDS))
      } else {
    ...
    /** Continues the computation of this future by taking the current future
      ...
      */
    def continueWith[S](cont: Future[T] => S): Future[S] = doContinueWith(f, cont)
    ...
    /** Continues the computation of this future by taking the result
      ...
      * The resulting future contains a value returned by `cont`.
      */
    def continue[S](cont: Try[T] => S): Future[S] = doContinue(f, cont)

  }

  def doContinue[T, S](f: Future[T], cont: (Try[T]) => S): Future[S] = {
    async {
      val done: T = await {
        f
      }
  ...
  def doContinueWith[T, S](f: Future[T], cont: (Future[T]) => S): Future[S] = {
    async {
      val done: T = await {
        f
      }
