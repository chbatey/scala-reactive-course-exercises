...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listener: Listener = this.createListener(relativePath)
    listener.start()
    val subscription: Subscription = Future.run() { ct =>
      Future {
        println("Waiting for next request")
        handleRequest(listener, ct, handler)
      }
    }
    subscription
  }

  def handleRequest(listener: Listener, ct: CancellationToken, handler: Request => Response) {
    ...
    nextRequest.onComplete {
      case Success((request, exchange)) => {
        ...
        Future {
          ...
          if (ct.nonCancelled) {
            println("Listening for another request")
            handleRequest(listener, ct, handler)
          } else {
      ...
      case Failure(ex) => throw new RuntimeException("Oh dear")
    }
  }
}
...
object NodeScala {
  ...
  object Listener {
    ...
    class Default(val port: Int, val relativePath: String) extends Listener {
      private val s = HttpServer.create(new InetSocketAddress(port), 1)
      ...
      def createContext(handler: Exchange => Unit) = {
        println(s"Adding context to http server: $relativePath")
        s.createContext(relativePath, new HttpHandler {
          def handle(httpxchg: HttpExchange) = {
            println("Received Http Exchange")
      ...
      def removeContext() = {
        println(s"Removing context $relativePath")
        s.removeContext(relativePath)
      }
    }
